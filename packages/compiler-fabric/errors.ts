...
function boundedMessage(message: string): string {
  const normalized = message.length === 0 ? "Fabric compilation failed." : message;
  return normalized.length <= 512 ? normalized : `${normalized.slice(0, 509)}...`;
}

export function boundedFabricError(
...
function copyError(value: unknown, expectedCode: DomainErrorCode): McdevError | undefined {
  ...
  return boundedFabricError(expectedCode, values.message, hasPath ? values.path as string : undefined);
}
...
function copyErrors(code: unknown, errors: unknown): readonly McdevError[] {
  if (!isDomainErrorCode(code) || isProxy(errors) || !Array.isArray(errors) ||
    ...
    return Object.freeze([mcdevError("INTERNAL_ERROR", "Fabric compilation failed safely.")]);
  }
  const length = Object.getOwnPropertyDescriptor(errors, "length")?.value;
  if (!Number.isSafeInteger(length) || length < 1 || length > 100 || Reflect.ownKeys(errors).length !== length + 1) {
    return Object.freeze([mcdevError("INTERNAL_ERROR", "Fabric compilation failed safely.")]);
  }
...
export class FabricCompilerError extends Error {
  ...
  constructor(code: DomainErrorCode, errors: readonly McdevError[]) {
    ...
    super(copied[0]?.message ?? "Fabric compilation failed safely.");
    this.name = "FabricCompilerError";
...
export function fabricCompilerError(
  code: DomainErrorCode,
  message: string,
  path?: string,
): FabricCompilerError {
  return new FabricCompilerError(code, [boundedFabricError(code, message, path)]);
}

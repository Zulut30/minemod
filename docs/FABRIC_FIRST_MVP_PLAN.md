...
| Fabric pack/compiler/fixtures | Pack revision 3 и compiler phase 1 генерируют и строго собирают scaffold, items, blocks, creative entries, shaped/shapeless/smelting recipes, models, blockstates, loot, localization, typed YACL config, Mod Menu entrypoint и server join-message binding | Добавить gameplay bindings/sync, item/tool/armor contract, tags/datagen, GameTests и hosted gates |
...
**Интеграционный статус на 23 июля 2026:** `compiler-fabric` phase 1 потребляет только trusted pack revision 3 и генерирует Gradle scaffold, `fabric.mod.json`, split main/client entrypoint, items, blocks, creative entries, shaped/shapeless/smelting recipes, item/block models, blockstates, self-drop loot и `en_us`. Shaped-рецепты имеют строгий контракт `pattern`/`key` 1×1–3×3, поддерживают ванильные и объявленные generated ingredients, `resultCount` 1–64 и детерминированный Minecraft 1.20.1 JSON; strict Gradle/JAR test дополнительно запускает dedicated server и проверяет отсутствие ошибок загрузки recipe resources. Trusted library resolver поддерживает обязательный YACL 3.5.0 и optional Mod Menu 7.2.2; unknown IDs, duplicates и неверная relation отклоняются. Для выбранной пары генерируются JSON5-backed `GeneratedConfig`, YACL screen и optional `modmenu` entrypoint. ModSpec v1 описывает bounded categories и boolean/integer/string options; defaults, Java integer range, string length, uniqueness и dependency profile проверяются до workspace. String option может единожды связаться с server-authoritative `player_join_message`; generated event adapter использует Fabric JOIN event и literal text. Custom recipes по-прежнему отклоняются без безопасного typed payload. Пока используются явно помеченные placeholder textures; tags и datagen ещё не реализованы, поэтому F1.1 остаётся открытым.

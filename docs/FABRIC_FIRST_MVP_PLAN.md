...
| Fabric pack/compiler/fixtures | Pack revision 3 и compiler phase 1 генерируют и строго собирают scaffold, items, blocks, creative entries, shapeless/smelting recipes, models, blockstates, loot, localization, typed YACL config и Mod Menu entrypoint | Связать config с gameplay/sync, расширить recipe contract, добавить tags/datagen, GameTests и hosted gates |
...
**Интеграционный статус на 23 июля 2026:** `compiler-fabric` phase 1 потребляет только trusted pack revision 3 и генерирует Gradle scaffold, `fabric.mod.json`, split main/client entrypoint, items, blocks, creative entries, shapeless/smelting recipes, item/block models, blockstates, self-drop loot и `en_us`. Trusted library resolver поддерживает обязательный YACL 3.5.0 и optional Mod Menu 7.2.2; unknown IDs, duplicates и неверная relation отклоняются. Для выбранной пары генерируются JSON5-backed `GeneratedConfig`, YACL screen и optional `modmenu` entrypoint. ModSpec v1 описывает bounded categories и boolean/integer/string options; defaults, Java integer range, string length, uniqueness и dependency profile проверяются до workspace, а реальная strict Gradle-сборка подтверждает generated classes внутри remapped JAR. Shaped/custom recipes отклоняются до расширения ModSpec полями pattern/key/custom payload. Пока используются явно помеченные placeholder textures; tags и datagen ещё не реализованы, поэтому F1.1 остаётся открытым.

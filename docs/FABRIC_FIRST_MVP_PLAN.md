...
| Fabric pack/compiler/fixtures | Pack revision 3 и compiler phase 1 генерируют и строго собирают scaffold, items, blocks, typed materials/tools/armor, generated equipment icons/wearable layers, equipment tags, creative entries, shaped/shapeless/smelting recipes, models, blockstates, loot, localization, typed YACL config, Mod Menu entrypoint и server join-message binding | Добавить reference-driven visual variants/human review, gameplay bindings/sync, общий tags/datagen, GameTests и hosted gates |
...
**Интеграционный статус на 23 июля 2026:** `compiler-fabric` phase 1 потребляет только trusted pack revision 3 и генерирует Gradle scaffold, `fabric.mod.json`, split main/client entrypoint, items, blocks, creative entries, shaped/shapeless/smelting recipes, item/block models, blockstates, self-drop loot и `en_us`. ModSpec v1 описывает bounded материалы с tool/armor properties, optional validated palette, optional `visualProfile` и предметы `sword`, `pickaxe`, `axe`, `shovel`, `hoe`, `armor`; компилятор создаёт Minecraft 1.20.1 `Tier`, `ArmorMaterial`, правильные item classes, вкладки Tools/Combat, handheld-модели, vanilla equipment tags, разные 16×16 pixel-art иконки и два 64×32 wearable armor layers. Профиль независимо комбинирует три семейства силуэта и четыре surface motif, сохраняя `balanced/clean` для старых спецификаций. При отсутствии палитры `assets-core` детерминированно выводит её из material ID; equipment-only output больше не содержит placeholder. Три version-pinned reference study для Mythic Metals, Simply Swords и Marium's Soulslike Weaponry продвигают только правила, подтверждённые независимыми проектами; reuse policy запрещает чужую геометрию, текстуры и distinctive design. Shaped-рецепты имеют строгий контракт `pattern`/`key` 1×1–3×3, поддерживают ванильные и объявленные generated ingredients, `resultCount` 1–64 и детерминированный JSON. Strict Gradle/JAR test собирает все equipment archetypes, проверяет PNG и ресурсы внутри remapped JAR, запускает dedicated server и подтверждает отсутствие ошибок загрузки recipe/tag resources. Trusted library resolver поддерживает обязательный YACL 3.5.0 и optional Mod Menu 7.2.2; unknown IDs, duplicates и неверная relation отклоняются. Для выбранной пары генерируются JSON5-backed `GeneratedConfig`, YACL screen и optional `modmenu` entrypoint. String option может единожды связаться с server-authoritative `player_join_message`; generated event adapter использует Fabric JOIN event и literal text. Custom recipes по-прежнему отклоняются без безопасного typed payload. Placeholder остаётся только у basic items/blocks; общий datagen и human visual approval ещё не реализованы, поэтому F1.1 остаётся открытым.

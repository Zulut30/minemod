...
| Fabric pack/compiler/fixtures | Pack revision 3 и compiler phase 1 генерируют и строго собирают scaffold, items, blocks, typed materials/tools/armor, equipment tags, creative entries, shaped/shapeless/smelting recipes, models, blockstates, loot, localization, typed YACL config, Mod Menu entrypoint и server join-message binding | Добавить authored equipment assets, gameplay bindings/sync, общий tags/datagen, GameTests и hosted gates |
...
**Интеграционный статус на 23 июля 2026:** `compiler-fabric` phase 1 потребляет только trusted pack revision 3 и генерирует Gradle scaffold, `fabric.mod.json`, split main/client entrypoint, items, blocks, creative entries, shaped/shapeless/smelting recipes, item/block models, blockstates, self-drop loot и `en_us`. ModSpec v1 теперь описывает bounded материалы с tool/armor properties и предметы `sword`, `pickaxe`, `axe`, `shovel`, `hoe`, `armor`; компилятор создаёт Minecraft 1.20.1 `Tier`, `ArmorMaterial`, правильные item classes, вкладки Tools/Combat, handheld-модели, armor layer paths и vanilla equipment tags. Shaped-рецепты имеют строгий контракт `pattern`/`key` 1×1–3×3, поддерживают ванильные и объявленные generated ingredients, `resultCount` 1–64 и детерминированный JSON. Strict Gradle/JAR test собирает все equipment archetypes, проверяет ресурсы внутри remapped JAR, запускает dedicated server и подтверждает отсутствие ошибок загрузки recipe/tag resources. Trusted library resolver поддерживает обязательный YACL 3.5.0 и optional Mod Menu 7.2.2; unknown IDs, duplicates и неверная relation отклоняются. Для выбранной пары генерируются JSON5-backed `GeneratedConfig`, YACL screen и optional `modmenu` entrypoint. String option может единожды связаться с server-authoritative `player_join_message`; generated event adapter использует Fabric JOIN event и literal text. Custom recipes по-прежнему отклоняются без безопасного typed payload. Пока используются явно помеченные placeholder textures, включая wearable armor layers; общий datagen ещё не реализован, поэтому F1.1 остаётся открытым.

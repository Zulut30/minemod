...
## Выполнение условий и post-integration remediation

Условия 1–6 выполнены для исходных frozen patches и exact integration commit `39aa194fe79d4f4ebf0a2153e4516b64be97849e`; authoritative combined dry-run patch имеет SHA-256 `a01d9c530752694a3862140c4a0a6e64cb566421932e9ee70b5a9ad672ed4b28`. Условие 7 на этом commit не прошло: оба первых GitHub-hosted runs завершились failure.

Hosted runner до успеха обнаружил три portability-дефекта, поэтому исходное решение не было растянуто на изменённый код. Итоговый remediation diff получил отдельный freeze SHA-256 `ed3867b29c203e2db5049d8be183136b0b37573cf9e249355a7b49cfe9ad5a7f`, formal score `0.81/0.85`, успешный `6/6` verification и итоговый semantic verdict `APPROVE`. Его два successful hosted runs на `5fd2bdb7a0a12dce2716c24479bfd301ab14a612` завершились до formal remediation run, поэтому это не буквальное выполнение исходного порядка freeze → verify/review → hosted CI. Root orchestrator ретроспективно принимает это отклонение только для exact remediation SHA и exact hosted commit; новое exact-hash исключение относится только к непройденному `min_patch_score`. Scorer сохранил ранний AF_UNIX capability failure worker sandbox и не агрегировал поздний успешный verification; формула также учитывает число файлов и размер patch, но именно capability failure опустил результат ниже `0.85`. Никакой code, documentation, security, ownership, applicability или test exception не принят.

Полный журнал failed runs, причин, точных commits, artifact hashes, verification и двух successful hosted runs находится в `phase-0-hosted-remediation.md`. Любой другой remediation SHA этим решением не покрывается.

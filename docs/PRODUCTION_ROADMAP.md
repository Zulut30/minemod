...
3. генерируют читаемый NeoForge-проект;
...
Первый GA не обязан одновременно поддерживать Fabric, Forge и Paper. Базовый GA target — production compatibility pack NeoForge 26.1.2 либо его явно утверждённый successor, если к моменту release старый target больше нельзя безопасно поддерживать.

Fabric, Forge, multi-loader export и Paper получают отдельные maturity gates. Они не должны задерживать NeoForge GA или создавать ложное обещание feature parity.
...
| Target | Один NeoForge production pack | Тот же pack или утверждённый successor | Явно опубликованная support matrix |
...
### P5. Fabric production adapter — параллельно после P1

Fabric не должен реализовываться через замену NeoForge imports.

**Entry:**

- ModSpec и codegen-core contracts стабильны;
- loader-specific API не протекает в canonical IR;
- NeoForge reliability не ухудшается из-за expansion work.
...
- не менее 30 Fabric beta projects завершены;
- 30 дней pack-specific SLO;
- feature/capability matrix явно фиксирует различия с NeoForge;
- migration между pack revisions проверена.

### P6. Forge adapter — после стабильного Fabric candidate
...
| Multi-loader scope explosion | Высокая / Высокое | NeoForge SLO падает из-за adapter work | Independent teams/gates, no parity promise | Product owner; pause expansion |
...
- подготовить Fabric architecture RFC и experimental basic fixture без production promises.
...
- точный GA Minecraft/NeoForge pack;

...
Пример ниже описывает целевой production IR с `schemaVersion: 1`. В Phase 0
заморожен строгий wire-контракт ModSpec/ArtSpec v0 с числовым
`schemaVersion: 0`; версия `1` не является его alias и вводится только через
явную версионированную миграцию.
